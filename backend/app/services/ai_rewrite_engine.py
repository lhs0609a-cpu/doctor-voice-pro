...
<모든 원고 공통 편집 규칙>
- 원본 정보와 검색 키워드가 이 글의 주제와 사실의 유일한 기준이다. 원본에 없는 질환·증상·부위·치료·수치·기간을 임의로 추가하지 않는다. 주제가 불명확하면 일반적인 다른 질환 글로 바꾸지 말고 원본의 범위 안에서만 설명한다.
- 제목과 첫 문단부터 검색 키워드와 원본 주제를 유지한다. 본문 전체에서 다른 질환이나 unrelated한 사례로 주제를 바꾸지 않는다.
- 한 글에서는 독자의 핵심 질문 하나만 다룬다. 문제 제시 → 핵심 정보 → 이유 → 실제 적용·관리 → 확인된 자료·사례 → 안심과 안내 순으로 직선적으로 쓴다.
- 각 문단은 앞 문단의 질문에 답한다. 관련 없는 주제를 중간에 추가하거나 백과사전처럼 정보를 나열하지 않는다.
- 제공된 근거에 없는 사람·기간·수치·후기·효과를 만들지 않는다. 실제 경험처럼 보이는 가상 환자, 의사의 경험담, 통계 수치를 쓰지 않는다. 관찰된 변화와 관리의 인과관계를 단정하지 않는다.
- 독자의 걱정을 인정하되 공포를 키우지 않는다. 위험을 말한 뒤에는 현재 할 수 있는 현실적인 방법과 개인차를 함께 설명한다.
- 병원 권유는 불안을 이용하지 말고, 어떤 상황에서 상담이 도움이 되는지 판단 기준을 제시한 뒤 선택형으로 안내한다.
- 권위는 어려운 말을 많이 쓰는 데서 만들지 않는다. 핵심 개념을 정확히 정의하고, 주장과 근거를 구분하며, 개인차와 한계를 함께 밝힌다. 논문·가이드라인·제공된 사례가 없으면 출처나 수치를 꾸며내지 않는다.
- 신뢰를 위해 확실한 사실, 합리적인 해석, 아직 알 수 없는 부분을 각각 구분해 쓴다. '연구로 입증됐다', '논문에 따르면' 같은 표현은 실제 근거가 제공된 경우에만 사용한다.
- 도입에는 과장된 광고 인사 대신 독자를 존중하는 짧은 인사를 상황에 맞게 넣을 수 있다. 인사는 한두 문장을 넘기지 않고 곧바로 독자의 고민으로 이어간다.
- 감동은 가상의 사연으로 만들지 않는다. 독자가 겪는 불편을 정확히 이해하고, 늦었다는 죄책감이나 막연한 공포를 덜어주는 문장으로 진정성을 만든다.
- 설득은 결론을 강요하는 방식으로 하지 않는다. 왜 이 설명이 필요한지, 어떤 경우에 도움이 되는지, 무엇을 확인한 뒤 선택할지 논리적으로 연결한다.
- 'SCI급', '세계 최고', '권위 있는 연구'처럼 검증할 수 없는 권위 과시는 쓰지 않는다. 제공된 근거가 논문이면 연구 설계·대상·한계까지 과장 없이 요약한다.
- 사람처럼 쓰되 연기하지 않는다. '제가 진료실에서 자주 봤습니다', '열에 아홉입니다'처럼 근거 없는 체험·통계를 사용하지 않는다. 추상적인 공감 문구를 반복하지 말고 독자가 실제로 겪는 장면을 한두 개만 구체적으로 쓴다.
- 본문 전체의 어투를 합니다·했습니다·입니다체로 통일한다.
- 한 문장에는 핵심 내용 하나만 담고 45자 안팎으로 쓴다. 60자를 넘기면 반드시 나눈다.
- 같은 종결어미와 '많은 분들이', '중요합니다', '도움이 됩니다' 같은 상투 표현을 연속해서 반복하지 않는다.
- 완성 원고는 다음 편집 요소를 빠짐없이 갖춘다.
  1) 인사: 첫 문장 또는 첫 두 문장 안에서 부담 없는 인사를 넣고 바로 독자의 고민으로 연결한다.
  2) 제목 후킹: 검색 키워드를 제목 앞쪽에 두고, 독자가 얻을 답을 구체적으로 예고한다. 자극적인 공포·과장 표현은 쓰지 않는다.
  3) 도입 후킹: 독자가 실제로 겪는 장면이나 질문으로 시작한다. 막연한 공감 문구를 반복하지 않는다.
  4) 감동: 가상의 환자 후기나 치료 경험담을 만들지 말고, 독자의 불안과 선택의 어려움을 정확히 이해하는 문장으로 감정을 만든다.
  5) 권위와 신뢰: 병원 고유 정보와 확인 가능한 출처를 주장에 연결한다. 출처가 없는 SCI 논문·수치·전문가 경험은 절대 지어내지 않는다.
  6) 비유: 어려운 개념을 이해시키는 데 꼭 필요한 경우에만 일상적인 비유를 한두 개 사용한다. 의학적 결론을 비유로 대신하지 않는다.
  7) 명료함과 가독성: 한 문장에 한 주장만 담고, 소제목은 질문 또는 답을 보여 주며, 긴 문단은 나눈다.
  8) 실행 안내: 독자가 오늘 할 수 있는 행동과 진료를 고려할 기준을 구체적으로 제시한다.
  9) 마무리 후킹: 첫 문단의 걱정에 답하면서 기억에 남는 한 문장으로 닫되, 예약을 재촉하거나 효과를 보장하지 않는다.
- 'SCI급', 'SCI 논문에 따르면' 같은 표현은 실제 SCI 논문 원문·서지정보가 제공된 경우에만 쓴다. 자료가 없으면 연구 권위를 암시하지 말고 확인된 사실과 한계를 설명한다.
- 원장님이 환자에게 직접 설명하는 목소리로 쓴다. 어려운 전문용어보다 일상적인 단어를 먼저 쓰고, 한 문장에 한 가지 뜻만 담는다.
- 실제 진료에서 확인하는 순서·질문·기록·관리 방법이 원본이나 프로필에 있으면 1인칭으로 구체적으로 설명한다. 제공되지 않은 환자 사례, 진료 횟수, 성공 경험, 통계는 만들지 않는다.
- '많은 분들이', '제가 자주 봤습니다', '저희는 특별합니다' 같은 추상적인 권위 문장을 반복하지 않는다. 원장님의 판단은 반드시 무엇을 보고 어떤 이유로 그렇게 판단하는지 뒤에 붙인다.
- 독자가 소리 내어 읽어도 자연스럽도록 문장 길이를 섞고, 문단 사이에는 앞 문장의 질문에 답하는 연결 문장을 둔다. 보고서식 나열과 광고 문구를 피한다.
- 글을 다 읽은 뒤 독자가 이 병원에 상담을 고려할 이유가 남도록 한다. 병원이 실제로 잘하는 판단 과정과 관리 방식을 독자의 문제 해결 흐름 안에서 보여 주고, 어떤 경우에는 다른 진료가 먼저일 수 있는지도 솔직하게 밝힌다. 방문을 재촉하거나 효과를 보장하지 않는다.
</모든 원고 공통 편집 규칙>
"""
...
<최종 점검>
출력하기 전에 다음을 모두 확인한다.
1. 제목과 모든 문단이 검색 키워드 '{keyword or '(없음)'}'와 원본 정보의 주제를 벗어나지 않는가.
2. 다른 질환·신체 부위·사례·통계가 새로 끼어들지 않았는가.
3. 근거 없는 의사 경험담, 환자 후기, 숫자, 치료 효과를 만들지 않았는가.
4. 첫 문단은 독자의 걱정을 정확히 짚고, 중간은 그 질문에 답하며, 끝은 불안을 낮추는 현실적인 다음 행동으로 닫히는가.
5. 문장이 짧고 자연스러운가. 같은 표현을 반복하거나 인공지능이 만든 듯한 상투어를 줄였는가.
6. 합니다·했습니다·입니다체가 끝까지 유지되는가.
조건을 하나라도 지키지 못하면 출력하기 전에 해당 문장을 고친다. 원본 정보에 없는 내용을 추가해 글을 풍성하게 만들지 않는다.
</최종 점검>"""
...
감동은 가상 환자 후기나 의사 체험담으로 만들지 않습니다. SCI급·논문·수치·권위는
제공된 자료가 있을 때만 언급하며, 자료가 없으면 절대 만들어내지 않습니다.
원장님이 환자에게 진료실에서 설명하듯 씁니다. 추상적인 권위 표현 대신 실제로 확인하는
순서와 판단 이유를 구체적으로 말합니다. 프로필과 원본에 없는 환자 사례나 경험은 만들지 않습니다.
독자가 한 번에 이해할 수 있는 쉬운 단어를 우선하고, 작성 후 소리 내어 읽었을 때 걸리는 문장과
반복되는 홍보 표현을 모두 고칩니다.
마지막에는 이 병원의 상담 방식이 특히 도움이 될 수 있는 사람과 먼저 다른 평가가 필요한 사람을
구분해 설명합니다. 독자가 스스로 적합성을 판단하게 하며, 그 판단 결과 상담을 고려할 충분한
이유가 자연스럽게 남도록 씁니다.
출력은 제목 한 줄과 본문만 작성합니다."""
        ...
        # 모델이 한글 입력을 자리표시자(?, ??? 등)로 바꾼 불완전한 원고는 저장하지 않는다.
        if re.search(r"\?{2,}|\bTODO\b|\[본문\]|\[병원명\]", content, flags=re.IGNORECASE):
            raise ValueError("Gemini가 자리표시자를 포함한 불완전한 원고를 반환했습니다. 원고를 저장하지 않습니다.")
