...
UNIVERSAL_EDITORIAL_RULES = """모든 원고에 적용하는 편집 규칙:
- 글을 쓰기 전에 독자의 핵심 질문 하나와 글의 결론 하나를 정한다. 서로 다른 고민을 한 글에 섞지 않는다.
- 문단은 문제 제시 → 핵심 정보 → 그 정보가 필요한 이유 → 실제 적용·관리 → 관련 사례·자료 → 마무리 순으로 직선적으로 전개한다. 주제에 따라 해당 단계가 없으면 생략하되 순서를 뒤섞지 않는다.
- 각 문단은 바로 앞 문단의 질문에 답한다. 방사형으로 새 주제를 꺼내거나 정보 목록을 나열하지 않는다.
- 제공된 근거에 없는 사람·기간·수치·후기·효과를 만들지 않는다. 관찰된 변화와 원인을 단정하지 않는다.
- 핵심 개념은 정확히 정의하고, 사실·해석·한계를 구분한다. 논문·가이드라인·사례가 제공되지 않았으면 출처나 수치를 만들지 않는다.
- 독자의 불안을 키우지 말고, 현실적으로 할 수 있는 방법과 선택 기준을 함께 제시한다. 가상의 감동 사례나 권위 과시는 금지한다.
- 설득은 결론을 강요하지 않고 문제와 근거와 선택지를 논리적으로 연결한다.
- 본문 전체의 어투를 하나로 통일한다. 기본은 합니다·했습니다·입니다체이며 해요체·반말을 중간에 섞지 않는다.
- 한 문장에는 핵심 내용 하나만 담고 45자 안팎으로 쓴다. 60자를 넘는 문장은 반드시 나눈다.
- 사진은 바로 앞 문단의 내용을 설명하는 경우에만 배치한다. 그래프·체력·근력 자료도 해당 지표를 설명한 문단에만 연결한다."
"""

# 키네스 원고는 일반 의료 블로그 프리셋보다 더 좁은 편집 규칙을 쓴다.
# 이 문자열은 Gemini에 직접 전달되므로 프리셋이 누락되거나 커스텀된 경우에도
# 지점 블로그 원고의 흐름과 표현을 동일하게 유지한다.
KINES_RULES = """키네스 지점 원고 편집 규칙:
- 기존 브리프에 검사 중심 흐름·필수 단어가 남아 있어도 아래 규칙을 우선한다.
- 글을 쓰기 전에 독자의 걱정 하나와 글의 결론 하나를 정한다. 서로 다른 고민을 한 글에 섞지 않는다.
- 문단 순서는 반드시 걱정 제시 → 함께 살펴볼 핵심 항목 → 그 항목이 중요한 이유 → 키네스에서 실제로 제공하는 관리 → 관련 사례·자료 → 처음 걱정으로 돌아가는 안내로 고정한다.
- 각 문단은 바로 앞 문단의 질문에 답한다. 방사형으로 새 주제를 꺼내거나 정보 목록을 나열하지 않는다.
- 사례는 제공된 근거 자료에서 확인된 내용만 사용한다. 환자 이름, 기간, 수치, 변화, 후기를 만들지 않는다. 관찰된 변화와 관리의 인과관계를 단정하지 않는다.
- 키네스가 뼈나이 확인, 골연령 검사, 성장판 검사를 직접 시행한다고 읽히는 표현을 쓰지 않는다. 해당 내용이 근거에 없으면 언급하지 않는다.
- 지점명은 '키네스 송도점', '키네스 분당점'처럼 쓴다. '키네스 센터 송도점' 표기는 쓰지 않는다.
- 어투는 본문 전체를 합니다·했습니다·입니다체로 통일한다. 해요·거든요·하시죠·반말을 섞지 않는다.
- 한 문장에 핵심 내용 하나만 담고 45자 안팎으로 쓴다. 60자를 넘는 문장은 반드시 나눈다.
- 그래프·체력·근력 자료는 그 지표를 설명한 문단 직후에만 연결한다. 본문과 관계없는 이미지를 넣기 위해 내용을 바꾸지 않는다."""


def _kines_rules(client: Dict[str, Any], brief: Optional[Dict[str, Any]]) -> str:
    name = str((client or {}).get("name") or "")
    brief_name = str((brief or {}).get("name") or "")
    return ("\n\n" + KINES_RULES) if ("키네스" in name or "키네스" in brief_name) else ""
...
- 사진은 바로 앞 대목의 내용을 설명해야 한다. 글에 없는 장소·장비·활동을 추정하지 않는다.
- 그래프·체력·근력 이미지는 해당 지표와 사례 기록을 직접 설명한 대목에만 배치한다. 키 성장과 무관한 그래프를 끼워 넣지 않는다.
- 보유 사진이 해당 지점 사진인지 확인할 수 없으면 지점 사진으로 단정하지 않는다.
