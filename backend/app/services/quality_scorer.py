...
# 근거·권위 신호
# 채점기는 근거가 없을 때 "'일반적으로 알려진 바로는' 처럼 쓰세요" 라고 권하면서
# 정작 그 표현을 세지 않았다. 권한 대로 썼는데 0점이면 모델은 출처를 지어내는 쪽으로 간다
# (2026-09-25 실측: 근거 0/5 가 다섯 번 연속). 권하는 표현을 세어 준다.
EVIDENCE_SIGNALS = re.compile(
    r"(학회|가이드라인|보건복지부|질병관리청|건강보험|심사평가원|의사협회|"
    r"연구(?:에|를|가|팀)|논문|임상|통계|조사\s*결과|자료에\s*따르면|"
    r"일반적으로\s*알려진|알려져\s*있는\s*바|보고되(?:고|어|는)|교과서적으로)"
)
