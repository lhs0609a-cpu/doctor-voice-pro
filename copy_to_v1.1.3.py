...
target_dir = source_dir.parent / "DoctorVoicePro-v1.1.3-20251030"
...
def should_exclude(path: Path) -> bool:
    ...
    if path.is_file():
        ...
        if 'doctorvoice.db' in path.name:
            return True
        if path.name == 'copy_to_release.py':
            return True
        if path.name == 'copy_to_v1.1.3.py':
            return True
...
# 개선된 배치 파일 복사
shutil.copy2(
    source_dir / "improved_start_backend.bat",
    target_dir / "start_backend.bat"
)
print("  [복사] start_backend.bat")

shutil.copy2(
    source_dir / "improved_start_frontend.bat",
    target_dir / "start_frontend.bat"
)
print("  [복사] start_frontend.bat")

shutil.copy2(
    source_dir / "install_and_start.bat",
    target_dir / "install_and_start.bat"
)
print("  [복사] install_and_start.bat")
