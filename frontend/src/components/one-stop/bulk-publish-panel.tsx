...
export function BulkPublishPanel({ campaignId, onRecurring, noImages = false }: { campaignId: string; onRecurring: () => void; noImages?: boolean }) {
  ...
  const start = async () => {
    ...
    try {
      setTask(await campaignAPI.startAutomation(campaignId, {
        // 3단계에서 '사진 없이 진행'을 골랐으면 글당 사진 0장으로 준비한다(사진이 없는데 3장을 요구하면 시작부터 막힌다).
        max_keywords: count, discover_keywords: true, quality: noImages ? { ...config, image_count: 0 } : config,
  ...
  return <Card className="space-y-5 border-primary/30 p-5">
    <div><h2 className="text-xl font-semibold">글 여러 개, 한 번에 준비하고 발행</h2>
      <p className="mt-2 text-sm text-muted-foreground">키워드는 자동으로 찾습니다. 검수를 통과한 글은 블로그별 발행 한도에 맞춰 나눠 예약합니다.</p></div>
    ...
    {!loaded ? <p role="status">{error || '저장된 설정을 불러옵니다…'}</p> : running ? <div className="space-y-3" role="status">
      ...
      <p className="text-xs leading-5 text-muted-foreground">최대 {count || 0}개 글을 씁니다. 품질 검사를 통과한 글만 예약하므로 실제 예약 수는 조금 적을 수 있어요. AI 사용 요금이 발생하며, 1번의 PC 실행기가 켜져 있어야 네이버에 올라갑니다.</p>
