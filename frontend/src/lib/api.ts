...
export const naverAPI = {
  ...
  // 크롬 확장 프로그램용 네이버 로그인 정보 관리
  saveCredentials: async (username: string, password: string): Promise<void> => {
    await api.post('/api/v1/naver/credentials', null, {
  ...
  getCredentials: async (): Promise<{ username: string; password: string } | null> => {
    try {
      const response = await api.get('/api/v1/naver/credentials')
      return response.data
    } catch (error: any) {
  ...
  deleteCredentials: async (): Promise<void> => {
    await api.delete('/api/v1/naver/credentials')
  },
}
