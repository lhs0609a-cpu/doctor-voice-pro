"""
Doctor Voice Pro - Universal Starter (최종실행파일4)
다른 컴퓨터에서도 정상적으로 백엔드와 프론트엔드 연결
"""
...
def start_frontend(port: int) -> subprocess.Popen:
    ...
    # Windows에서는 npm.cmd 사용 또는 shell=True
    if sys.platform == 'win32':
        process = subprocess.Popen(
            'npm run dev',
            stdout=subprocess.PIPE,
...
def main():
    ...
    print_header("Doctor Voice Pro - Universal Starter")
    print("  다른 컴퓨터에서도 정상 작동!")
    print_header("")
